...
class Controlador:
    def __init__(self, page: ft.Page):
        ...
        print(
            f"[Controlador INIT]: pauta_start_timestamp={self.pauta_start_timestamp}, current_pauta_text={self.current_pauta_text}"
        )
        self.page.go("/")
        ...
        try:
            ...
            if len(partes) == 2:
                ...
                if nova_pauta_texto != self.current_pauta_text:
                    ...
                    print(
                        f"[entrar_na_votacao_como_votante]: NOVA PAUTA '{nova_pauta_texto}'. pauta_start_timestamp set to {self.pauta_start_timestamp}"
                    )
                else:
                    print(
                        f"[entrar_na_votacao_como_votante]: MESMA PAUTA '{nova_pauta_texto}'. pauta_start_timestamp={self.pauta_start_timestamp} (não resetado)."
                    )
                ...
                print(
                    f"[entrar_na_votacao_como_votante]: AVISO: Formato inesperado da mensagem da pauta: '{mensagem_recebida}'"
                )
            ...
            print(
                f"[entrar_na_votacao_como_votante]: ERRO: Não foi possível extrair o tempo da mensagem da pauta: '{mensagem_recebida}'"
            )
            ...
            print(
                f"[entrar_na_votacao_como_votante]: ERRO: Erro inesperado ao processar mensagem da pauta: {ex}"
            )
    ...
    def start_voter_countdown(self):
        print(
            f"[start_voter_countdown]: Chamado. pauta_start_timestamp atual={self.pauta_start_timestamp}, tempo_votacao={self.tempo_votacao}"
        )
        if (
            ...
            print(
                f"[start_voter_countdown]: Não iniciando. timer_control_votante={self.timer_control_votante}, tempo_votacao={self.tempo_votacao}, pauta_start_timestamp={self.pauta_start_timestamp}"
            )
        ...
        print(
            f"[start_voter_countdown]: Iniciando NOVA thread do timer. pauta_start_timestamp={self.pauta_start_timestamp}"
        )
    ...
    def _voter_countdown_task(self):
        ...
        print(
            f"[_voter_countdown_task {thread_id}]: Thread iniciada. pauta_start_timestamp={self.pauta_start_timestamp}"
        )

        if self.pauta_start_timestamp is None:
            current_time = self.tempo_votacao
            print(
                f"[_voter_countdown_task {thread_id}]: AVISO: pauta_start_timestamp é None. Usando tempo_votacao como fallback."
            )
        else:
            elapsed_time = int(time.time() - self.pauta_start_timestamp)
            current_time = max(0, self.tempo_votacao - elapsed_time)
            print(
                f"[_voter_countdown_task {thread_id}]: Tempo inicial calculado: {current_time}s (tempo_votacao={self.tempo_votacao}, decorrido={elapsed_time})"
            )
        ...
        while current_time >= 0 and not self.stop_timer_event.is_set():
            ...
            if self.timer_control_votante:
                ...
                if self.timer_control_votante.value != new_value:
                    ...
                    try:
                        ...
                        print(
                            f"[_voter_countdown_task {thread_id}]: Falha na atualização da UI: {e}. Parando thread."
                        )
            ...
            if self.pauta_start_timestamp is not None:
                ...
                current_time -= 1
                print(
                    f"[_voter_countdown_task {thread_id}]: pauta_start_timestamp é None no loop. Decrementando manualmente."
                )
